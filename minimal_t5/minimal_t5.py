...
class T5Model(nn.Module):
    ...
    def init_kv_cache(self, decoder_input_ids):
    ...
    def generate(self, encoder_input_ids, generation_length):
        ...
        for layer in self.decoder.layers:
            ...
            precomputed_kv_hidden_states.append({
                "key": layer.cross_attention.k(encoder_hidden_states).view(*out_shape).transpose(1, 2),
                "value": layer.cross_attention.v(encoder_hidden_states).view(*out_shape).transpose(1, 2),
            })
        ...
        kv_cache = self.init_kv_cache(decoder_input_ids)
...
class T5Attention(nn.Module):
    ...
    def forward(self,
        ...
        scores = torch.matmul(
            query_states, key_states.transpose(3, 2).type_as(query_states)
        )
        if attention_bias is None:
            # Enc-Dec
            scores += attention_mask
        else:
            scores += attention_bias + attention_mask
